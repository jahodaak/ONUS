...
const service = {
  ...
  broadcasts: () => FIND_MANY_RESULT,
  broadcast: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Broadcast", () => {
  ...
  test("GET /broadcasts/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/broadcasts"}/${nonExistingId}`)
  ...
  test("GET /broadcasts/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/broadcasts"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /broadcasts existing resource", async () => {
